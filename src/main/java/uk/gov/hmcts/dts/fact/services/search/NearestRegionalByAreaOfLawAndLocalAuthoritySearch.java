...
@Component
public class NearestRegionalByAreaOfLawAndLocalAuthoritySearch implements Search {

    private final CourtWithDistanceRepository courtWithDistanceRepository;

    public NearestRegionalByAreaOfLawAndLocalAuthoritySearch(final CourtWithDistanceRepository courtWithDistanceRepository) {
    ...
    public List<CourtReferenceWithDistance> search(final MapitData mapitData, final String postcode, final String areaOfLaw) {
        return courtWithDistanceRepository
            .findNearestRegionalByAreaOfLawAndLocalAuthority(mapitData.getLat(), mapitData.getLon(), areaOfLaw, postcode)
            .stream()
